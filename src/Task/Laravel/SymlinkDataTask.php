...
class SymlinkDataTask extends Task implements ShellCommandServiceAwareInterface
{
    ...
    public function execute(Node $node, Application $application, Deployment $deployment, array $options = [])
    {
        ...
        foreach ($options['symlinkDataFolders'] as $directory) {
            ...
            $commands[] = sprintf(
                'ln -sf %1$s %2$s',
                escapeshellarg($relativeDataPathFromWeb . '/' . $directory),
                escapeshellarg($absoluteWebDirectory . '/' . $directory)
            );
        }
    ...
    protected function resolveOptions(OptionsResolver $resolver)
    {
        ...
        $resolver->setNormalizer('directories', static function (Options $options, $value) {
            if (is_array($value)) {
                return $value;
            }
        ...
        $resolver->setNormalizer('webDirectory', static function (Options $options, $value) {
            return trim($value, '\\/');
        });
    }
}
